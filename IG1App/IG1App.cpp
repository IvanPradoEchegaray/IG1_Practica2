...
void IG1App::run()   // enters the main event processing loop
{
	if (mWinId == 0) { // if not initialized
		mLastUpdateTime = glutGet(GLUT_ELAPSED_TIME);	//inicializar tiempo de la primera actualizacion
...
void IG1App::update() {
	//actualizar valor de la ultima vez que se actualiza la app
	GLuint frameTime = glutGet(GLUT_ELAPSED_TIME) - mLastUpdateTime;
	//si sobrepasa el tiempo de actualizacion por frame
	if (frameTime >= MILLISECS_PER_FRAME && animationUpdate) {
		mScene->update();
		glutPostRedisplay();
...
{  // double buffering
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);  // clears the back buffer, similar to a Screen Clear
