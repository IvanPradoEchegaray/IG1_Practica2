...
GridCube::GridCube(GLdouble lado, GLuint nDiv, Texture* topTex, Texture* sideTex) {
	//Bottom
	Grid* bot = new Grid(lado, nDiv);
	bot->setTexture(topTex);
	//SideBack
	Grid* back = new Grid(lado, nDiv);
	...
	left->setModelMat(rotate(left->modelMat(), radians(-90.0), dvec3(0.0, 1.0, 0.0)));
	//SideRight
	Grid* right = new Grid(lado, nDiv);
	...
	right->setModelMat(rotate(right->modelMat(), radians(-90.0), dvec3(0.0, 1.0, 0.0)));
	//SideFront
	Grid* front = new Grid(lado, nDiv);
	...
	//Top
	Grid* top = new Grid(lado, nDiv);
	top->setTexture(topTex);
	dvec3 postop = dvec3(0, 200.0, 0);
	top->setModelMat(translate(dmat4(1), postop));
